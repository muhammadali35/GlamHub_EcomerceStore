...
const Favorites = () => {
  ...
  return (
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-md flex flex-col items-center justify-center p-10 text-center max-w-md">
      <AiFillHeart className="text-gray-400 text-6xl mb-4" />
      ...
      <Link
        to="/"
        className="mt-6 inline-block bg-black text-white px-6 py-2 rounded-full hover:bg-brand-gold hover:text-black transition"
      >
        Browse Products
      </Link>
    </div>
            ...
            {favList.map((product) => (
              ...
              >
                ...
                <div className="relative overflow-hidden bg-gray-100">
                  ...
                  {/* View Button */}
                  <Link to={`/product/${product.category}/${product._id}`}>
                    <button className="absolute top-14 right-3 bg-white text-black p-2 px-3 rounded-full shadow opacity-0 group-hover:opacity-100 transition duration-500 hover:bg-black hover:text-white">
                      👁
                    </button>
                  </Link>
